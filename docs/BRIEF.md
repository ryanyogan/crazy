...
| The Shell at both widths, the More screen, a route behind every destination                                                                                                                           | Real; Invoices is still an empty route                                                                                                                                                                                                                                                                 |
...
| Visual comparison harness (`pnpm visual`): frames 1a, 1c–1g, 2a, 2b, 3a and 3b against their routes, derived phone screenshots                                                                        | Real; frames 2c and 4a join with the tickets that draw those screens                                                                                                                                                                                                                                   |
...
| The Billing module: `billing.set`, Client and Time entry in the schema, the Cori persona (`POST /dev/seed?persona=cori`)                                                                              | Real. The Shell gains and loses Time and Invoices at once, and both routes answer Not found with it off. At most one running Time entry is held by a partial unique index, and `decide` holds the same rule (ticket 17). Frames 2a, 2b, 3a and 3b are compared; 2c and 4a wait on their tickets        |
...
| Time screen: the timesheet a Day, a Week or a Month at a time, read by `readTime` and derived by `viewTime`; editing an entry in place, adding one by hand, and confirming the Client Crazy suggested | Real on seeded rows. Every figure is counted from the period's own rows at the moment the screen is read; the period lives in the URL. The accounting targets are drawn and not wired (ticket 23), and the column frame 2b gives to Invoices holds the period's figures until ticket 21                |
...
- **The Time screen on a phone.** Frame 2b draws desktop only. One column: the heading with the
  ...
  line, the note and Edit on the second — then the flag, the place to add an entry and the period's
  figures. A row has no room for an editor inside it there, so the same editor rises from the
  bottom edge in the `Sheet` primitive.
...
- **The flag under the timesheet.** Frame 2b's tag says "2 entries need a project" and its sentence
  ...
  sentence promises.
- **Where the Time screen's accounting targets are drawn.** Frame 2b puts "Synced to · QuickBooks ·
  09:00" in the foot of the rail; the Shell follows frame 1a there on every screen (Live, then the
  user), so the targets sit on the Time screen itself, in the column frame 2b gives to Invoices,
  and say plainly that nothing has been sent (ticket 23). That column also holds the period's own
  figures — tracked, billable, not billable — until the Invoices screen is built (ticket 21).
