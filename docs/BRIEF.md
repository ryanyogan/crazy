...
| Visual comparison harness (`pnpm visual`): frames 1c–1g, 2b, 2c, 3a, 3b and 4a against their routes, frames 1a and 2a part by part (`Part.crop`), derived phone screenshots                                                                                                       | Real                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Today screen: greeting, date, Brief, the day ribbon and six chapters — Catch up, Meetings, What's left, Your day, This week by Client (the Billing module on) and Later this week — read from D1 by `readToday` and derived by `viewRundown`                                      | Real on seeded rows. Every chapter's count and its one sentence is a pure function over the day's rows and the moment; Take on now, the Priority stack, the timeline, the Mentions and "This week by Client" are the frames' own parts, inside the chapters (ADR 0003). Start and Swap are wired on the desktop, where the frame draws them                                                                                                                                                                                       |
...
| The Brief's text, a meeting's prep note (`meeting_prep`), the stack's order and each Todo's reason                                                                                                                                                                                | Seeded, stored where generated text will live; a meeting with no row shows no note at all                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Billing and accounting on the Integrations screen, and the invoice settings per Client (`client.setInvoicing`, migration 0014)                                                                                                                                                    | Real, with the Billing module on. Xero is connected through Clerk exactly as the other Providers are (`oauth_xero`); QuickBooks, Harvest, Stripe, FreshBooks and Toggl Track are drawn and plainly unavailable, because Clerk brokers no connection to them. Cadence, payment terms, auto-draft and send without review are commands on the Client; moving a Client's terms moves the due day of an invoice she has not sent. Nothing syncs and nothing is sent: auto-draft and send without review are real settings that say so |
...
| The Billing module: `billing.set`, Client and Time entry in the schema, the Cori persona (`POST /dev/seed?persona=cori`)                                                                                                                                                          | Real. The Shell gains and loses Time and Invoices at once, and both routes answer Not found with it off. At most one running Time entry is held by a partial unique index, and `decide` holds the same rule (ticket 17). Frames 2a, 2b, 2c, 3a, 3b and 4a are compared                                                                                                                                                                                                                                                            |
...
- **The whole of the Today screen, at both widths** (ADR 0003). At the owner's direction Today is a
  rundown: the day line, the heading and the Brief as frames 1a and 2a draw them, then the day
  ribbon — the working day on one 28px rule, a meeting an accent tint, a Slot a hairline box, a free
  hour dashed, and with the Billing module on what was tracked drawn solid along the foot — then a
  reading column of chapters, each a disclosure whose head is a kicker, a count and one sentence
  derived from the day's rows. Beside it on a desktop, a sticky aside 340px wide (frame 1a's own
  width for the cards in it) holding Take on now with the Billing module off, the place to add a
  Todo, and the index of the chapters, which marks the one in view. On a phone one column: the
  index as a row of chips under the ribbon, Take on now above the chapters, the place to add a Todo
  at the foot. What is inside a chapter is the frames' own part, unchanged; what stopped being
  comparable and why is written beside the targets in `tools/visual/src/targets.ts`.
...
- **The Integrations screen on a phone.** The frame draws desktop only. One column: the Providers
  one under another, then — with the Billing module on — the billing and accounting Providers and
  the invoice settings per Client, then the Account, then Realtime and the lifecycle settings.
- **Where the billing section and the Client settings sit on the desktop.** Frame 2c draws them as
  though they were the whole screen. The billing Providers are a section of the Integrations
  screen, under the Providers every user has; the invoice settings head the right-hand column,
  where 2c draws them and at 2c's own 360px (frame 1g's column, with the module off, is 340px).
- **The per-Client controls, and turning on send without review.** Frame 2c draws auto-draft and
  send without review once under the list of Clients. They are the Client's, like the cadence and
  the terms: each Client's group carries its own pair, its own cadence and its own payment terms.
  Send without review is off unless she turns it on, and turning it on asks a second time in place
  — "Really let Meridian Health's invoices go out unread?" — which is the screen's one solid accent
  fill. Turning it off again takes one press.
