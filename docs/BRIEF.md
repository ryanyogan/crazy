...
| The command seam (`decide` and `apply` in `@crazy/shared`), `useCommand`, the Coordinator's single write path                                                                    | Real, for fourteen commands (the Billing module on or off among them): complete, add, snooze a Todo, give it a Slot or take it off, start it or swap it; a Mention; the lifecycle settings and time zone, a Connection and its Side; the Rollover; a Source reported complete   |
...
| The Billing module: `billing.set`, Client and Time entry in the schema, the Cori persona (`POST /dev/seed?persona=cori`)                                                         | Real. The Shell gains and loses Time and Invoices at once, and both routes answer Not found with it off. At most one running Time entry is held by a partial unique index; the command-seam rule arrives with the timer (ticket 17). The Cori frames have not been compared yet |
...
| Backlog ageing and where a range's Todos came from                                                                                                                               | Real: counted from Todos at the moment the screen is read, over `todo(userId, state, touchedAt)` and `todo(userId, createdAt)`                                                                                                                                                  |
...
- Still to come, per the spec: phone layouts for Projects, Time and Invoices.

Six deliberate differences from the frames: the wordmark reads CRAZY where they read TODAY; the
...
Circles this week" where frame 1e says "Four groups this week"; it says Circles are inferred
from "which Providers the work lives in" where the frame says "which tools"; and the Metrics
screen says "Backlog ageing" where frame 1f says "Backlog aging", because the repo writes British
English. The glossary lists every one of the frame's wordings as one to avoid. The harness draws
the frames with all six changes (`WORDMARK` and `COPY` in `tools/visual/src/targets.ts`).
